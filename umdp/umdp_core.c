...
// Fix the check_if_open_file_is_netlink_socket_with_port_id function around line 535-555
static int check_if_open_file_is_netlink_socket_with_port_id(
    ...
    int err;
    struct partial_netlink_sock* nlk;
    u32 expected_port_id;

    socket = sock_from_file(file, &err);
    if (!socket) return 0;
    if (socket->ops->family != PF_NETLINK) {
        return 0;
    }

    nlk = container_of(socket->sk, struct partial_netlink_sock, sk);
    printk(KERN_DEBUG "umdp: found netlink socket with portid %u:\n", nlk->portid);

    expected_port_id = *(u32*) v;
