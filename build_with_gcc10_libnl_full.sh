...
# First, let's check if your libnl was built correctly
ls -la "$HOME/riscv-libnl/lib/"
ls -la "$HOME/riscv-libnl-static/lib/" 2>/dev/null || echo "Static libnl not found"

# Update your build script with fixes
cat > build_with_gcc10_libnl_full_fixed.sh << 'EOF'
#!/bin/bash
set -e
...
chmod +x build_with_gcc10_libnl_full_fixed.sh